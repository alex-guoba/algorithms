...
// GetEdge will return all adjacent vertices without wight
func (g *Graph) GetAdjacent(v int) (adjs []int, ok bool) {
	var vertexs map[int]int

	if vertexs, ok = g.edges[v]; !ok {
		return
	}

	adjs = make([]int, len(vertexs))
	i := 0
	for v := range vertexs {
		adjs[i] = v
		i++
...
// GetEdges get all edges with weight
func (g *Graph) GetEdges() [][]int {
	result := [][]int{}

	for vetex, edges := range g.edges {
		for edge, weight := range edges {
			if !g.Directed {
				if vetex < edge {
					result = append(result, []int{edge, vetex, weight})
				}
			} else {
				result = append(result, []int{edge, vetex, weight})
			}
